...
class GAME(simpleGE.Scene):
    def __init__(self):
        ...
        self.lblComputerScore = LblComputerScore()
        self.sprites = [self.hand, self.btnShoot, self.lblScore, self.lblComputerScore]

    def update(self):
        if self.userScore < 7 and self.computerScore < 7:
            if self.btnShoot.clicked:
                ...
                for Hands in self.hand:
                    ...
                    if result == "win":
                        ...
                        self.lblComputerScore.text = f"Computer: {self.computerScore}"
        else:
            if self.userScore == 7:
...
class LblComputerScore(simpleGE.Label):
    def __init__(self):
        super().__init__()
        self.center = (540, 50)
        self.text = "Computer: 0"
